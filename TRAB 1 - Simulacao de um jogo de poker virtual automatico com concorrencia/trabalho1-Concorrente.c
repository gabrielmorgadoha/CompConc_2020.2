...
/* Codigo: Trabalho de Implementacao 1 - Simulacao de um jogo de poker virtual automatico com concorrencia */
...
int nthreads, combinacoes[10], jogadores = 0;
long int nJogadores;
...
void contaCombinacoes(int pos){
    jogadores++; // conta o numero de jogadores processados pelas threads para checagem posterior
    ...
    if(strcmp(vetorJogadores[pos].combinacao, nomeCombinacoes[0]) == 0){  
        pthread_mutex_lock(&mutex); // bloqueia o acesso quando uma thread detecta a combinacao para evitar erros de contagem em uma variavel global
        combinacoes[0]++;
        pthread_mutex_unlock(&mutex); // libera para outras threads o acesso a esta parte ao terminar o acrescimo
    }
...
// executa de forma concorrente o processo de verificar a combinacao adquirida por cada jogador
void *tarefaCombinacao(void *args){
    // variaveis utilizadas para execucao da tarefa das threads | a tarefa foi divida em blocos para cada uma das threads
    long int i, fim, tamBloco = nJogadores/nthreads, inicio = (long int) args * tamBloco;
    char stringAux[26];
    FILE *file;

    if((long int) args == nthreads - 1){
        fim = nJogadores;
    }
    else{
        fim = inicio + tamBloco;
    ...
    pthread_mutex_lock(&mutex); // bloqueia o acesso durante a abertura do arquivo para garantir que nao ocorram problemas pelas threads tentaram abrir o mesmo arquivo diversas vezes
    ...
    pthread_mutex_unlock(&mutex); // libera o acesso ao terminar de abrir o arquivo

    // pula linhas no arquivo texto para chegar ate o ponto de inicio desejado
    for(i = 0; i < inicio; i++){
        fgets(stringAux, 26, file);
    }
    ...
    pthread_mutex_lock(&mutex); // bloqueia o acesso durante o fechamento do arquivo para garantir que nao ocorram problemas pelas threads tentaram fechar o mesmo arquivo diversas vezes
    fclose(file); // fecha o arquivo
    pthread_mutex_unlock(&mutex); // libera o acesso ao terminar de fechar o arquivo
    pthread_exit(NULL);
}
...
int main(int argc, char *argv[]){
    ...
    pthread_mutex_init(&mutex, NULL); // inicializa o mutex (lock de exclusao mutua) a ser usado nas threads

    // recebe o numero de jogadores e de threads a serem usados no programa pela linha de codigo. Aborta a execucao caso nao receba os parametros esperados
    if(argc < 3){
        fprintf(stderr, "Digite: %s <numero de jogadores> <numero de threads>\n", argv[0]);
        return 1;
    }
    ...
    for(k = 0; k < nthreads; k++){
        if(pthread_create(tid + k, NULL, tarefaCombinacao, (void *) k)){
    ...
    // printa o numero jogadores processados pelas threads e o total de combinacoes feitas | caso sejam iguais as threads contaram corretamente
    ...
    */
    ...
    // libera a memoria alocada para combinacoes, vetor de jogadores e threads
    for(i = 0; i < nJogadores; i++){
